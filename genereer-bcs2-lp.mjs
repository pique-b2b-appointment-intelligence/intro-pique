// BCS Batch 2 - persoonlijke landingspagina's generator (journey / verhaallijn-model)
// Output: bcs2/[slug].html  ->  intro-pique.agency/bcs2/[slug]
// Bron: bcs-batch2-base.json + bcs-batch2-research.json + bcs2-content.json (geschreven per prospect)
// Design: geport van pique/iclicks.html (geanimeerde verhaallijn langs hoofdstuk-dots).
// Model: Agents/landingspagina-agent.md  (Akt 1 herkenning 30% / Akt 2 spanning 40% / Akt 3 brug 20% / Akt 4 bewijs+CTA 10%)
// Video Jeffrey blijft bovenaan. Merk-tegel per merk uit bcs2-favicons/.
// Schrijfregels: geen em-dashes, geen zin die met "Geen" begint, geen emojis/symbolen.
...
const content = JSON.parse(readFileSync(new URL('./bcs2-content.json', import.meta.url)));
const css = readFileSync(new URL('./bcs2-style.css', import.meta.url), 'utf8');
const oldDrop = new Set(['Amsterdenim', 'Posie Beauty', 'Willicroft', 'The Limonaderij']);
// merken met een blanco/leeg favicon: houd het monogram
const favExclude = new Set(['a-little-lovely-company', 'kaerel-skincare', 'passion-for-linen']);
...
const firstName = naam => {
  if (!naam) return '';
  const t = naam.trim().split(/[\s,&]+/)[0];
  if (/^[A-Za-z]\.?$/.test(t)) return '';
  return t;
};

// merk-tegel + browser-favicon: base64 data-URI uit bcs2-favicons/[slug].(png|ico), anders monogram
const favDir = new URL('./bcs2-favicons/', import.meta.url);
const favData = (s, naam) => {
  if (!favExclude.has(s)) {
    for (const ext of ['png', 'ico']) {
      const f = new URL(`${s}.${ext}`, favDir);
      if (existsSync(f)) {
        const mime = ext === 'png' ? 'image/png' : 'image/x-icon';
        const uri = `data:${mime};base64,${readFileSync(f).toString('base64')}`;
        return { logo: `<div class="bt-logo bt-img"><img src="${uri}" alt="${esc(naam)}"></div>`, icon: uri, mime };
      }
    }
  }
  return { logo: `<div class="bt-logo">${esc(mono(naam))}</div>`, icon: null, mime: null };
};

// Akt 1 - bevindingen als hoofdstukken langs de lijn (right/left alternerend)
...
// Akt 2 - betekenis-alinea's binnen één hoofdstuk
const betekenisParas = (paras) => (paras || []).map(p => `<p class="ch-text">${esc(p)}</p>`).join('');

const page = p => {
  const r = p.r;
  const naam = p.bedrijf;
  const c = content[naam];
  if (!c) return null; // geen geschreven content -> overslaan
  const s = p._slug;
  ...
  const meta = [p.stad, p.vert, '2026'].filter(Boolean).map(esc).join(' &middot; ');
  const fav = favData(s, naam);
  const iconLink = fav.icon
    ? `<link rel="icon" type="${fav.mime}" href="${fav.icon}">\n<link rel="apple-touch-icon" href="${fav.icon}">`
...
<main class="journey" id="journey">
  <svg class="journey-path" id="jpath" preserveAspectRatio="none" aria-hidden="true">
    ...
    <section class="chapter center phase">
      <div class="ch-inner">
        <span class="ch-dot"></span>
        <div class="ch-body"><div class="ch-eyebrow">Wat we zagen</div><h2 class="ch-kop ch-head">Bij ${esc(naam)}.</h2><p class="ch-sub">Drie dingen die ons opvielen. Ze horen bij elkaar.</p></div>
      </div>
    </section>
${findingChapters(c.findings)}
    ...
    <section class="chapter left">
      <div class="ch-inner">
        <span class="ch-dot"></span>
        <div class="ch-body"><div class="ch-eyebrow">Wat dit betekent</div><h2 class="ch-kop">De prijs van stilzitten.</h2>${betekenisParas(c.betekenis)}<p class="ch-pull">${esc(c.pull)}</p><p class="ch-text" style="margin-top:1.4rem;">${esc(c.imagine)}</p></div>
      </div>
    </section>
...
// genereren
mkdirSync(new URL('./bcs2/', import.meta.url), { recursive: true });
const index = [];
const seen = new Set();
let written = 0, skipped = [];
for (const p of rows) {
  let s = slug(p.bedrijf);
  while (seen.has(s)) s += '-2';
  seen.add(s);
  p._slug = s;
  const html = page(p);
  if (!html) { skipped.push(p.bedrijf); continue; }
  writeFileSync(new URL(`./bcs2/${s}.html`, import.meta.url), html);
  index.push({ bedrijf: p.bedrijf, slug: s, url: `intro-pique.agency/bcs2/${s}`, beslisser: p.r.naam || '', verificatie: p.r.conf });
  written++;
}
writeFileSync(new URL('./bcs2-index.json', import.meta.url), JSON.stringify(index, null, 1));
console.log(`Gegenereerd: ${written} landingspagina's in bcs2/. Overgeslagen (geen content): ${skipped.length}${skipped.length ? ' -> ' + skipped.join(', ') : ''}`);
