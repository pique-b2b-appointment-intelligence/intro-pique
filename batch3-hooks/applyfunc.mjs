import fs from 'fs';
const D='/Users/simonkempers/Desktop/Pique - B2B Appointment Intelligence/Klanten/BCS/batch3-hooks/';
const OUT='/Users/simonkempers/Desktop/Pique - B2B Appointment Intelligence/Klanten/BCS/';
const all=JSON.parse(fs.readFileSync(D+'FINAL-assembled.json','utf8'));
const func=JSON.parse(fs.readFileSync(D+'func-results.json','utf8'));
const norm=s=>(s||'').toLowerCase().replace(/[^a-z0-9]/g,'');
const fJA=new Map(), fLayer=new Map();
for(const x of func){
  const k=norm(x.web)||norm(x.bedrijf);
  if(x.gevonden==='JA'&&x.func_naam){ if(!fJA.has(k)) fJA.set(k,x); }
  else if(/bestaat wel|aantoonbaar|functionele laag|head of ecommerce|head of e-commerce|cs-functie|customer service .*manager|customer happiness/i.test(x.toelichting||'')){ fLayer.set(k,x); }
}
let applied=0, flagged=0;
for(const r of all){
  const k=norm(r.web)||norm(r.bedrijf);
  r.founder_fallback='';
  if(fJA.has(k)){
    const x=fJA.get(k);
    r.founder_fallback=(r.beslisser_naam||'')+(r.beslisser_functie?' ('+r.beslisser_functie+')':'');
    r.beslisser_naam=x.func_naam; r.beslisser_functie=x.func_rol; r.beslisser_status='GEVERIFIEERD';
    r.notitie=(r.notitie?r.notitie+' | ':'')+'functionele beslisser (founder=fallback)';
    applied++;
  } else if(fLayer.has(k)){
    r.notitie=(r.notitie?r.notitie+' | ':'')+'CS/Ops-laag bestaat, naam via telefoon achterhalen';
    flagged++;
  }
}
fs.writeFileSync(D+'FINAL-assembled.json', JSON.stringify(all,null,1));
console.log('functionele beslisser toegepast:',applied,'| CS-laag-vlag gezet:',flagged);

// rebuild CSV
const full=r=>r.adres_zeker==='JA'&&r.beslisser_status==='GEVERIFIEERD'&&r.beslisser_naam&&r.hook;
const rank=r=>full(r)?0:(r.adres_zeker==='JA'&&r.hook?1:2);
all.sort((a,b)=>rank(a)-rank(b)||(a.bedrijf||'').localeCompare(b.bedrijf||''));
const esc=s=>'"'+String(s==null?'':s).replace(/"/g,'""').replace(/\r?\n/g,' ')+'"';
const split=n=>{if(!n)return['',''];n=String(n).replace(/\s*&.*$/,'').trim();const p=n.split(/\s+/);return[p[0]||'',p.slice(1).join(' ')];};
const head=['bedrijf','website','voornaam','achternaam','functie','beslisser_status','founder_fallback','straatnaam','postcode','plaatsnaam','land','adres_zeker','hook','hook_bron','notitie'];
let csv=head.map(esc).join(',')+'\n';
for(const r of all){const[vn,an]=split(r.beslisser_naam);csv+=[r.bedrijf,r.web,vn,an,r.beslisser_functie,r.beslisser_status,r.founder_fallback,r.straat,r.postcode,r.plaats,r.land,r.adres_zeker,r.hook,r.hook_bron,r.notitie].map(esc).join(',')+'\n';}
fs.writeFileSync(OUT+'BCS-Batch3-Prospects-Beslisser-Hook-Adres.csv', csv);
console.log('CSV herbouwd:',all.length,'rijen | VOLLEDIG',all.filter(full).length,'| functionele beslisser',all.filter(r=>r.founder_fallback).length);
